...
public interface BoardMapper {
	...
	@Select(value = {"select * from (select ROWNUM rnum, board.* from (select * from BOARD where MEM_EMAIL = #{mem_email} order by B_RDATE desc) board) where rnum > #{startRow} and rnum <= #{endRow} and MEM_EMAIL = #{mem_email}"})
